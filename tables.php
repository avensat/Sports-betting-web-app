<?php
	...
	mysqli_query($connect,$sql) or die('could not create table'.mysql_error());
	...
	$sql_4= 'create table if not exists league(
	id_league int primary key auto_increment not null,
	league varchar(225)
	)';	
	mysql_query($sql_4,$connect) or die('could not create'.mysql_error());
	if($sql_4){echo 'successfully';}
	
	
	$sql_5= 'create table if not exists odds(
	...
	over varchar(225),
	under varchar(225))';
	mysql_query($sql_5,$connect) or die('could not create'.mysql_error($connect));
	if($sql_5){echo 'successfully';}
	
	
	$sql_6= 'create table if not exists club(
	id_club int primary key auto_increment not null,
	home varchar(225),
	away varchar(225))';
	mysql_query($sql_6,$connect)or die('could not create'.mysql_error($connect));
	if($sql_5){echo 'successfully';}
	...
	$sql_8= 'create table if not exists country(
	id_score int primary key auto_increment not null,
	country varchar(225))';
	mysql_query($sql_8,$connect) or die('could not connect'.mysql_error($connect));
	if($sql_7 ==TRUE){echo "success";}
	elseif($sql_7==FALSE){echo 'unsuccessful';}
	*/
	/*
	...
	*/
		...
		/*
	$sql_11= "create table if not exists betslip(
		...
		$query= mysql_query($sql_11,$connect) or die('could not create'.mysql_error());
		if($query){echo 'successfully';}
